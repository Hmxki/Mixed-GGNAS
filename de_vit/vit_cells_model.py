...
from UNAS_Net.cell_module.cells import darts_genotypes
from UNAS_Net.cell_module.cells.build_darts_cell import Build_Darts_Cell
# from UNAS_Net.models.mamba_block import MB_BLOCK
from UNAS_Net.utils.early_stopping import EarlyStopping
import torch.optim as optim
from UNAS_Net.utils.encodings import *
from UNAS_Net.cell_module.buildcells import buildcell
from UNAS_Net.models.vit_block import transformer
...
class vit_CellModel(nn.Module):

    def __init__(self, chromosome=None, model_config=None,config=None,w_tensor=None,init_c=3,num_classes=2,base_c=32,device='cuda',
                 img_size=(256,256),  dropout_rate=0.1, attention_dropout_rate=0, retrain = False, new_weight=None):
        super(vit_CellModel, self).__init__()
        # CONSTANT
        self.ALPHA = 0.6
        self.BETA = 0.4
        self.NUM_VERTICES = 7
        ...
        self.dim = [128,256,512,768]
        self.heads = [4,8,16,32]
        self.depth = [6,8,10,12]
        self.mlp_dim = [512,768,1024,1536]
        self.patch_size = [8,16,32]


        # 单元内部权重
        #self.cells_weight = nn.ParameterList([nn.Parameter(w_tensor[i]) for i in range(8)]).to(self.device)
        ...
        self.transformer = transformer(img_size, self.depth[self.config_vit[4]], self.dim[self.config_vit[0]], self.mlp_dim[self.config_vit[3]],
                                       dropout_rate, self.heads[self.config_vit[2]], attention_dropout_rate,patches=self.patch_size[self.config_vit[1]])

        # if self.config_vit[1] == 0:
        #     self.conv2d_trans = nn.Conv2d(self.dim[self.config_vit[0]], self.out_channels[3], 3, padding=1)
        # elif self.config_vit[1] == 1:
        #     self.conv2d_trans = nn.ConvTranspose2d(self.dim[self.config_vit[0]], self.out_channels[3], 2,stride=2)

        if self.config_vit[1] == 0:
            self.conv2d_trans = nn.Conv2d(self.dim[self.config_vit[0]], self.out_channels[3], 3, padding=1,stride=2)
        elif self.config_vit[1] == 2:
            self.conv2d_trans = nn.ConvTranspose2d(self.dim[self.config_vit[0]], self.out_channels[3], 2,stride=2)
        else:
            self.conv2d_trans = nn.Conv2d(self.dim[self.config_vit[0]], self.out_channels[3], 3,padding=1)
        ...
        self.decode = [nn.Conv2d(self.out_channels[4 + i], self.num_classes, kernel_size=1).to(self.device) for i in
                       range(3)]
    ...
    def evaluate(self, train_loader, val_loader, loss_fn, metric_fn, device):
        ...
        for epoch in range(min_epoch, 50 - min_epoch):
            # Train Phase
            self.train()
        ...
        with open(f"../results/idrid_vit/model_{self.solNo}.txt", "w") as f:
            f.write(log)
