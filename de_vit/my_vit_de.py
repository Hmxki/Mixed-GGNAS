...
from UNAS_Net.utils.distances import *

def readPickleFile_best(file):
    with open(f"../best_model/model_{file}.pkl", "rb") as f:
        data = pickle.load(f)

    return data

def readPickleFile(file):
    with open(f"H:\\hmx\\NAS_data\\idrid\\models_vit_de\\model_{file}.pkl", "rb") as f:
        data = pickle.load(f)

    return data

def readPickleFile_trials(file):
    with open(f"H:\\hmx\\NAS_data\\idrid\\trials_vit_de\\model_{file}.pkl", "rb") as f:
        data = pickle.load(f)
...
class MY_VIT_DE():
    ...
    def __init__(self, train_dataloader,
        ...
        self.best_modelNo = best_modelNo
    ...
    def load_pretrained_model(self):
        if self.pretrain_model is None:
            ...
            self.pretrain_model = readPickleFile_best(self.best_modelNo)
    ...
    def writePickle(self, data, name):
        # Write History
        with open("H:\\hmx\\NAS_data\\idrid\\models_vit_de\\model_{}.pkl".format(name), "wb") as pkl:
            pickle.dump(data, pkl)

    def writePickle_de(self, data, name):
        # Write History
        with open("H:\\hmx\\NAS_data\\idrid\\trials_vit_de\\model_{}.pkl".format(name), "wb") as pkl:
            pickle.dump(data, pkl)
    ...
    def init_population(self, pop_size=None):
        ...
        while i < pop_size:
            ...
            model = vit_CellModel(chromosome, self.model_config, config, new_weight=self.w_tensor,
                                  init_c=self.init_c, device = self.device, img_size=self.img_size, retrain=True)
            # Same Solution Check
    ...
    def init_eval_pop(self):
        ...
        for i in range(self.pop_size):
            ...
            # 预训练模型状态字典
            pretrained_state_dict = {k: v for k, v in self.pretrain_model.state_dict().items() if
            ...
            self.log_dic['pretrain_model'] = self.best_modelNo
            with open('../log/check_point_idrid_vit.txt', 'w') as f:
    ...
    def evolve_generation(self):
        ...
        # mutation -> crossover
        # 如果是断点，读取断点处保存的进化个体
        if self.check_point and len(os.listdir('H:\\hmx\\NAS_data\\idrid\\trials_vit_de'))!=0:
            for i in range(self.pop_size):
                # 读取断点处的所有进化个体
            ...
            for j in range(self.pop_size):
                ...
                model = vit_CellModel(trial, self.model_config, config, new_weight=self.w_tensor,
                                      init_c=self.init_c, device=self.device, img_size=self.img_size, retrain=True)
                model.solNo = self.solNo
                self.solNo = 1 + self.solNo
                trials.append(model)
        ...
        for j in range(self.model_numb%self.pop_size,self.pop_size):
            ...
            isSameSolution, sol = self.checkSolution(mutant.config_vit)
            if isSameSolution:
                print("SAME SOLUTION",sol)
                self.writePickle(mutant, mutant.solNo)
                self.allModels[mutant.solNo] = list(mutant.config_vit)
                ...
                model_dict.update(pretrained_state_dict)
                # 加载新的状态字典
                mutant.load_state_dict(model_dict)
    ...
    def run(self, seed, check_point, pop_size):
        ...
        if self.check_point and len(os.listdir('H:\\hmx\\NAS_data\\idrid\\models_vit_de'))!=0:
            self.reset()
            f = open('../log/check_point_idrid_vit.txt', 'r')
            self.log_dic = ast.literal_eval(f.read())
            self.model_numb = self.log_dic['solNo'] + 1
            self.best_modelNo = self.log_dic['pretrain_model']
            ...
            # 如果不是在初始化种群中断，则继续进化
                ...
                while self.totalTrainedModel < self.MAX_SOL:
                    ...
                    self.generation = 1 + self.generation
                    self.log_dic['generation'] = self.generation
                    with open('../log/check_point_polyp_vit_wo_darts.txt', 'w') as f:
                        f.write(str(self.log_dic))
        elif self.check_point and len(os.listdir('H:\\hmx\\NAS_data\\idrid\\models_vit_de'))==0:
            self.seed = seed
            self.solNo = 0
            self.generation = 0
