...
def uncertainty_loss(input,target):
    preudo_label = F.softmax(target / 0.5, dim=1).detach()
    preudo_label = torch.argmax(preudo_label,dim=1).detach()
    vanilla_loss = F.cross_entropy(input,preudo_label,reduction='none')

    kl_div = torch.sum(F.kl_div(F.log_softmax(input,dim=1), F.softmax(target,dim=1).detach(),reduction='none'),dim=1)
    uncertainty_loss_ = (torch.exp(-kl_div) * vanilla_loss).mean() + kl_div.mean()
    return uncertainty_loss_
...
class criterion(nn.Module):
    ...
    def forward(self, inputs, target, loss_weight=None, num_classes: int = 2, dice: bool = True,
        ...
        for name, x in inputs.items():
            ...
            if dice is True:
                ...
                dice_ = dice_loss(x, dice_target, multiclass=True, ignore_index=ignore_index)
                loss = dice_ + loss
