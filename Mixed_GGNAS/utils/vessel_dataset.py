...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Read Image
        image = read_image(os.path.join(self.img_dir, self.mode, self.images.iloc[idx]['Name']))[0,:,:]

        # Read Label
        lbl_name = os.path.join(self.lbl_dir, self.mode, self.labels.iloc[idx]['Name'])
        label = read_image(lbl_name)[0,:,:]
    	
        if self.transform:
            image = self.transform(image).reshape((1,128,128))
        if self.target_transform:
            label = self.target_transform(label).reshape((1,128,128))
