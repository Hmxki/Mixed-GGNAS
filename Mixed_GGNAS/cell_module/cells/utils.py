...
from Mixed_GGNAS.cell_module.cells.encoder_colors import get_mask_pallete
...
def save_checkpoint(state, is_best, save):
    filename = os.path.join(save, 'weights.pt')
...
def get_gpus_memory_info():
    ...
    rst = rst.strip().split('\n')
    memory_available = [int(float(line.split(':')[1].split(' ')[1])) for line in rst if 'Free' in line][::2]
